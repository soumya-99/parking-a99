...
const CreateReceiptScreen = ({navigation, route}) => {
  ...
  useEffect(() => {
    console.log(
      receiptSettings,
      'receiptSettingsxxxxxreceiptSettingsreceiptSettings',
    );
  ...
  const handleCreateReceipt = async () => {
    ...
    if (carindata?.status) {
        ...
        try {
          ...
          await ThermalPrinterModule.printBluetooth({
            payload:
              ...
              `[L]<font size='normal'>IN TIME : [R]${formatDateTime(
                currentTime,
              )}</font>\n` +
      ...
      // For PAX A99
        ...
        if (generalSettings.adv_pay == 'Y') {
          advanceAmount += `[L]ADVANCE : [R] ${vehicleAdv}\n`;
        }
        ...
        payload +=
          ...
          // `[C]<img>https://synergicportal.in/syn_header.png</img>\n` +
          `[C]====================\n` +
          `[L]RECEIPT NO : [R] ${receipt_number}\n` +
          `[L]VEHICLE TYPE. : [R] ${type}\n` +
          `[L]VEHICLE NO : [R] ${vehicleNumber}\n` +
          // `[L]<font size='normal'>ADVANCE : [R] ${vehicleAdv}</font>\n` +
          ...
          `[L]IN TIME : [R]${formatDateTime(currentTime)}\n` +
          // `[C]<u><font size='small'>${receiptNoObj}</font></u>\n` +
